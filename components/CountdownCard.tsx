...
import { Goal, TimeLeft } from '../types';
...
const calculateTimeLeft = (endDate: string): TimeLeft => {
  ...
  let timeLeft: TimeLeft = { days: 0, hours: 0, minutes: 0, seconds: 0, totalMilliseconds: 0 };
...
const CountdownCard: React.FC<CountdownCardProps> = ({ goal, onToggleComplete }) => {
  const [timeLeft, setTimeLeft] = useState<TimeLeft>(calculateTimeLeft(goal.endDate));
  ...
  useEffect(() => {
    ...
    if (timeLeft.totalMilliseconds <= 0) {
      // Si el tiempo ya terminó al cargar, asegurar que el progreso sea 100%
      setProgress(100);
      return;
    }
    ...
    const timer = setInterval(() => {
      ...
      if (newTimeLeft.totalMilliseconds <= 0) {
        clearInterval(timer);
        setProgress(100); // Asegurar 100% cuando el tiempo termina
      }
    }, 1000);

    return () => clearInterval(timer);
  }, [goal.startDate, goal.endDate, goal.isCompleted, timeLeft.totalMilliseconds]); // timeLeft.totalMilliseconds para reiniciar si es necesario (aunque el efecto principal es por goal)
